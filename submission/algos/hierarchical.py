...
class HierarchicalTorchPolicy(rllib.policy.TorchPolicy):
    ...
    def _log_learning_rates(self):
        ...
        # for algo_idx, algo in enumerate(self.algorithms):
        #     key = f"algo{algo_idx}"
        #     v = log.log_learning_rate(algo)
        #     if key not in self._to_log.keys() and v != {}:
        #         self._to_log[key] = {}
        #     self._to_log[key].update(v)

        for algo_idx, algo in enumerate(self.algorithms):
            if algo_idx == self.active_algo_idx:
                key = f"algo{algo_idx}"
                if key in self.to_log.keys():
                    self._to_log[f"algo{algo_idx}"].update(
                        log.log_learning_rate(algo)
                    )
                else:
                    self._to_log[f"algo{algo_idx}"] = log.log_learning_rate(
                        algo
                    )

    def _log_weight_norm(self):
        """
        Use to log LR to check that they are really updated as configured.
        """
        # for algo_idx, algo in enumerate(self.algorithms):
        #     key = f"algo{algo_idx}"
        #     v = log.log_weight_norm(algo)
        #     if key not in self._to_log.keys() and v != {}:
        #         self._to_log[key] = {}
        #     self._to_log[key].update(v)
        for algo_idx, algo in enumerate(self.algorithms):
            if algo_idx == self.active_algo_idx:
                ...
                if key in self.to_log.keys():
                    self._to_log[f"algo{algo_idx}"].update(
                        log.log_weight_norm(algo)
                    )
                else:
                    self._to_log[f"algo{algo_idx}"] = log.log_weight_norm(algo)
