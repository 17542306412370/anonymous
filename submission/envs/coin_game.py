...
def swap_two_actions(action, idx1, idx2):
    if action == idx1:
        return idx2
    elif action == idx2:
        return idx1
    ...
    # idx1_mask = actions == idx1
    # actions[actions == idx2] = idx1
    # actions[idx1_mask] = idx2
    #
    # return actions


def flip_state(observations, axis):
    return np.flip(observations, axis=axis)


class CoinGame(
    OtherPlayMixin, InfoAccumulationInterface, MultiAgentEnv, gym.Env
):
    """
    ...
    SYMMETRIES = [
        {"name": "identity", "obs_sym": identity, "act_sym": identity},
        {
            "name": "flip_h",
            "obs_sym": partial(flip_state, axis=[-2]),
            # "obs_sym": partial(torch.flip, dims=[-2]),
            "act_sym": partial(swap_two_actions, idx1=0, idx2=1),
        },
        {
            "name": "flip_v",
            "obs_sym": partial(flip_state, axis=[-3]),
            # "obs_sym": partial(torch.flip, dims=[-3]),
            "act_sym": partial(swap_two_actions, idx1=2, idx2=3),
        },
        {
            "name": "flip_hv",
            "obs_sym": partial(flip_state, axis=[-3, -2]),
            # "obs_sym": partial(torch.flip, dims=[-3, -2]),
            "act_sym": partial(
                partial(swap_two_actions, idx1=2, idx2=3), idx1=0, idx2=1
            ),
        },
    ]
    ...
    def _load_config(self, config):
        ...
        self._use_other_play = config.get("use_other_play", False)
    ...
    def reset(self):
        ...
        rllib_obs = {self.player_red_id: obs[0], self.player_blue_id: obs[1]}
        rllib_obs = self._apply_other_play(rllib_obs, obs=True)
        return rllib_obs
    ...
    def step(self, actions: Dict):
        ...
        """
        actions = self._apply_other_play(actions, acts=True)
    ...
    def _to_RLLib_API(self, observations, rewards):
        ...
        obs = self._apply_other_play(obs, obs=True)
