...
class MatrixSequentialSocialDilemma(
    OtherPlayMixin, InfoAccumulationInterface, MultiAgentEnv, ABC
):
    """
    ...
    def _to_RLLib_API(
        ...
        observations = self._apply_other_play(observations, obs=True)
...
def flip(input):
    ...
    # new_input[...,0]=
    # reset = input == 5
    # last_act_pl1 = input % 2
    # last_act_pl2 = input // 2
    # last_act_pl1 = (last_act_pl1 + 1) % 2
    # last_act_pl2 = (last_act_pl2 + 1) % 2
    # input = last_act_pl1 + last_act_pl2 * 2
    # input[reset] = 5
...
class IteratedBoS(
    ...
    SYMMETRIES = [
        {"obs_sym": identity, "act_sym": identity},
        {
            "obs_sym": flip,
            # "obs_sym": partial(torch.flip, dims=[-2, -1]),
            "act_sym": reverse_actions,
        },
    ]
