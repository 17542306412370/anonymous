...
from submission.algos.augmented_ppo import MyPPOTorchPolicy
...
def modify_hyperparams_for_the_selected_env(hp):
    ...
    if "CoinGame" in hp["env_name"]:
        ...
        if "ABCoinGame" in hp["env_name"]:
            hp["plot_keys"] += ab_coin_game.PLOT_KEYS
            hp["plot_assemblage_tags"] += ab_coin_game.PLOT_ASSEMBLAGE_TAGS
...
def _modify_config_for_policy_gradient(
    ...
    if hp["use_policy_gratient"]:
        ...
        if eval:
            rllib_config["sgd_minibatch_size"] = 100
